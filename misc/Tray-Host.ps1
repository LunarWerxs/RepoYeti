# =====================================================================================
# LunarWerx shared TRAY-HOST engine (Windows). One parameterized WinForms tray host that
# runs a Bun (or any) daemon with NO console window and shows a tray icon with
# Open / Rebuild & Restart / Restart / Quit — the shared, superset evolution of the four
# near-identical per-app tray scripts (ReDesign / RepoYeti / DevWebUI / CC Manager UI).
#
# SHARED LunarWerx tray-host (source of truth: lunarwerx-ui/src/tray-host/Tray-Host.ps1;
# synced into each app's misc/ by sync.mjs, alongside the .vbs / Create-Shortcut.ps1 /
# <App>-Tray.ps1 ADAPTER). DO NOT EDIT THIS FILE INSIDE AN APP — edit the kit copy and run
# `node sync.mjs`. The per-app <App>-Tray.ps1 is now a THIN ADAPTER: it keeps its
# `param([int]$Port = <port>, [switch]$SelfTest)`, builds a $TrayConfig hashtable, dot-sources
# this engine, then calls Invoke-TrayHostSelfTest or Start-TrayHost. All app-specific behavior
# lives in that config; all shared machinery lives here.
#
...
#
# Why this exists as ONE engine: the four scripts had drifted into ~500-650 lines each of
# 95%-identical WinForms + background-runspace + watchdog code, with only a handful of genuine
# divergences (health service-id, mutex naming, shutdown protocol, dev-tree gating, launch
# command). Extracting the shared body into a config-driven engine means a fix (or a new
# behavior) lands once and syncs everywhere, and the per-app files shrink to a declaration of
# just what actually differs. Every WHY-comment from the originals is preserved/adapted below.
#
# Responsiveness (preserved from all four): "Rebuild & Restart" and "Restart" run their slow
# work (build, stop, port wait / graceful shutdown, readiness poll — up to ~a minute on a large
# RepoYeti scan root) on a BACKGROUND runspace, and a WinForms timer marshals the result back to
# the UI thread, so the tray never freezes (no "Not Responding"). Daemon control is STATELESS —
# it locates the live instance via the runtime pointer + /api/health and acts on the bound port
# (or, for the token apps, asks the daemon to shut itself down) — so the exact same helper
# functions run identically on the UI thread AND inside the worker runspace, surviving repeated
# restarts, with zero dependence on a shared Process handle.
#
# -------------------------------------------------------------------------------------
