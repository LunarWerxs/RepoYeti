...
import { initFileLogging, restoreFileLogging, logFilePath } from "../src/log-file.ts";

const dirs: string[] = [];
function tempHome(): string {
  const d = mkdtempSync(join(tmpdir(), "repoyeti-log-"));
  dirs.push(d);
  return d;
}
...
test("tees console.log/warn/error to logs/daemon.log with level tags, while the console still prints", () => {
  ...
  const path = initFileLogging({ dir: home });
...
test("initFileLogging is idempotent — a second call keeps the same path and doesn't double-patch", () => {
  const home = tempHome();
  const first = initFileLogging({ dir: home });
  const beforeLog = console.log;
  const second = initFileLogging({ dir: home });
