 ...
 * with CreateNoWindow, so its stdout/stderr go to a hidden console and are LOST. When the
 * process dies (the last-resort handlers in src/index.ts call console.error then exit(1) on
 * any uncaught throw/rejection) there was no record of WHY. This captures every console line
 * to a file that survives the process, regardless of how the daemon was launched (tray,
 * terminal, or the auto-update successor).
 *
 * Writes are SYNCHRONOUS (fs.writeSync on an appended fd), deliberately: a buffered stream
 * would lose the final console.error when the crash handler calls process.exit(1) a tick
...
// Same resolution as config.ts's CONFIG_DIR, inlined so this module can run as the very
// first thing at startup without importing the config/secrets chain. Keep in sync with
// config.ts (REPOYETI_HOME override, else ~/.repoyeti).
const defaultDir = (): string => process.env.REPOYETI_HOME ?? join(homedir(), ".repoyeti");
