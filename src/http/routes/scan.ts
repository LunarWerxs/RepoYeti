...
export function register(app: Hono, _deps: Deps): void {
  ...
  app.post("/api/scan", async (c) => {
    ...
    if (rawPath) {
      ...
      // Fire-and-forget: progress/failure streams over SSE (scan_progress/scan_done), the
      // HTTP response only acknowledges that the scan started.
      if (!isScanning()) void rescanFolder(abs).catch(() => {});
      return c.json({ ok: true, running: true, scope: "folder" });
    }
    // Same as above — errors surface over SSE, not in this response.
