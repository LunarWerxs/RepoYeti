...
export function register(app: Hono, { cfg }: Deps): void {
  ...
  app.get("/api/buzz", (c) => c.json({ config: buzzConfigView(cfg) }));

  app.put("/api/buzz", async (c) => {
  ...
  app.post("/api/buzz/communities", async (c) => {
  ...
  app.delete("/api/buzz/communities/:id", (c) => {
  ...
  app.post("/api/buzz/preflight", async (c) => {
