...
async function putMode(c: Context, cfg: Deps["cfg"]) {
...
async function applyTunnelHostname(t: NonNullable<Deps["cfg"]["tunnel"]>, hostname: string | undefined) {
...
async function applyTunnelToken(t: NonNullable<Deps["cfg"]["tunnel"]>, token: string | undefined) {
...
async function putTunnel(c: Context, cfg: Deps["cfg"]) {
  const p = await parseBody(c, TunnelSettingsSchema);
  if (!p.ok) return p.res;
  cfg.tunnel ??= {};
  const t = cfg.tunnel;
  await applyTunnelHostname(t, p.data.hostname);
  await applyTunnelToken(t, p.data.token);
...
function applyRelayFields(
  r: NonNullable<Deps["cfg"]["relay"]>,
  data: { url?: string; enabled?: boolean },
): void {
  if (data.url !== undefined) {
    const u = data.url.trim().replace(/\/+$/, "");
    if (u) r.url = u;
    else delete r.url;
  }
...
async function putRelay(c: Context, cfg: Deps["cfg"]) {
  const p = await parseBody(c, RelaySettingsSchema);
  if (!p.ok) return p.res;
  cfg.relay ??= {};
  const r = cfg.relay;
  applyRelayFields(r, p.data);
  if (r.enabled) {
...
export function register(app: Hono, { cfg }: Deps): void {
  app.put("/api/mode", (c) => putMode(c, cfg));
  app.put("/api/tunnel", (c) => putTunnel(c, cfg));
  app.put("/api/relay", (c) => putRelay(c, cfg));
}
