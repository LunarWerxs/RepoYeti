...
/**
 * Start the Cloudflare tunnel (idempotent). The URL arrives asynchronously: it's
 * broadcast over SSE and exposed at /api/status when cloudflared yields it. `onReady`
 * lets the CLI print it (with a QR) without coupling this module to the terminal.
 */
export function startManagedTunnel(onReady?: (url: string) => void): void {
