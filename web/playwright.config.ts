...
// a live daemon — start BOTH before `bunx playwright test` in `web/`, since the SSE live-update flow
// needs the real daemon (one-time browser install: `bunx playwright install chromium`). baseURL is
// overridable via PLAYWRIGHT_BASE_URL. Kept out of the Vitest run (vitest.config.ts excludes
