...
// saved placement are both ignored (verified Edge 150, 2026-07-16). Nothing outside the
// page can fix that, so the launcher tags the URL with the size the window should have and
// the page corrects itself here, once, at startup.
//
// The param name and the "WxH" shape are a contract shared with the daemon
// (src/window-size.ts) and the tray (misc/Tray-Host.ps1) — keep the three in step. RepoYeti
// has no web+server shared module, so the literal and the parser live here too rather than
// being imported (the daemon holds its own copies).
