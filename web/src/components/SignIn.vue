...
// /oauth/login (the PKCE dance). When the request is local, we also offer the local-continue
// path below (its label is the `signIn.continueLocal` message) — a loopback-only bypass, shipped
// as the finished feature, so the owner is never locked out at their own desk.
import { ref } from "vue";
