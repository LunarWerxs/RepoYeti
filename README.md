...
Nothing is mirrored or uploaded to RepoYeti's servers. Git traffic goes to your own remotes with your own credentials. If you use AI Smart Commit, AI commit messages, or AI conflict resolution, the changed file list and diff (or the conflicted text) is sent to the AI provider you configured, under your own key, and only when you trigger it. Optional settings sync sends dashboard preferences (never code, credentials, or paths) to Connections. Beyond that there is a small, anonymous daily ping to Connections Studio for update checks: a random install id, the app version, and a coarse OS tag. The server derives a coarse location, ASN, locale, and a truncated user agent from that, but never an IP address, hostname, username, file path, or anything about your repos. Set `REPOYETI_NO_PING=1` to opt out of the ping.

**Do I need to sign up for an AI provider to use Smart Commit?**
Yes, some key is required since RepoYeti doesn't ship one (Groq revokes any key committed to a public repo). Groq is the suggested provider: free, and set up takes about three clicks from Settings → AI. OpenAI, Claude, Gemini, OpenRouter, and DeepSeek also work if you'd rather use those. Keys live in your OS keychain, are never shown to the dashboard, and are sent only to the provider they belong to.
