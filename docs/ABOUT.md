...
- The Windows tray icon comes from a separate native Rust launcher process that spawns the daemon; running repoyeti.exe by itself never produces a tray icon, which reads as a bug the first time you see it. anchors: `misc/tray-host-native/src/main.rs:802`
...
- **System-tray launcher (Windows)** - A separate native Rust launcher (lunarwerx-tray) draws the tray icon and spawns the unchanged daemon binary; running repoyeti.exe alone never produces a tray icon. - `misc/tray-host-native/src/main.rs:802`
